...
const Profile: React.FC = () => {
  ...
  const [mode, setMode] = useState<'view' | 'edit' | 'list' | 'register'> ('list');
        ...
        {mode === "list" && userData ? (
          <div className="w-5/6">
            <table className="min-w-full bg-gray-300 border border-gray-300">
              <thead className=''>
                <tr className=''>
                  <th className="py-2 px-4 border text-left text-sky-900">ID</th>
                  <th className="py-2 px-4 border text-left text-sky-900">First Name</th>
                  <th className="py-2 px-4 border text-left text-sky-900">Last Name</th>
                  <th className="py-2 px-4 border text-left text-sky-900">Actions</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="py-2 px-4 border text-slate-900">{userData.id}</td>
                  <td className="py-2 px-4 border text-slate-900">{userData.firstName}</td>
                  <td className="py-2 px-4 border text-slate-900">{userData.lastName}</td>
                  <td className="py-2 px-4 border text-right">{renderActions(userData)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="overflow-scroll w-4/5">
            {selectedRegisteredUserResponse && (
              <UserForm
                ...
                setListMode={() => setMode("list")}
              />
            )}
          </div>
        )}
      </div>
      <hr className='border border-stone-900 shadow-blue-700 shadow-2xl mb-0 -ml-2'/>
      <div className="border-2 z-20 bg-stone-900 p-7 -ml-2"></div>
    </div>
  );
};
